...
from readFP_3D import readFP3D
...
class LSM:
    ...
    def _gen_func_list(self, func):
        # vectorizeは入力の値を引数として計算してくれる　返り値はリスト（ベクトル）
        # 値を直接ベクトル化するわけではなく、vrctorize(myfunc)とすることで、引数をintやstrからintのlistやstrのリストに変更することができる
        if func == "poly":
...
# 3DのFPを求める（２つ）
# 変換行列Mを適当に設定
# FP＝M＊FP'に近づくように最小二乗法を用いて更新

# x_train y_train x_test y_testを作成
# LSMを呼ぶ、fit predict plot
def main():
    FPDict1 = readFP3D("./FP_3D/FP_IMG_4465.txt")
    FPDict2 = readFP3D("./FP_3D/FP_IMG_4466.txt")
    #FPList1 = sorted(FPDict1.items())
    #FPList2 = sorted(FPDict2.items())
    X_train, y_train = [], []
    for key, value1 in FPDict1:
        value2 = FPDict2[key]
        for i in range(3):
            X_train[i].append(value1[i])
            y_train[i].append(value2[i])

    loss = calcLoss(X_train, y_train,)
