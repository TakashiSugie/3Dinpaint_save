...
def create_mesh(depth, image, int_mtx, config):
    H, W, C = image.shape
    ext_H, ext_W = H + 2 * \
        config['extrapolation_thickness'], W + \
        2 * config['extrapolation_thickness']
    LDI = netx.Graph(H=ext_H, W=ext_W, noext_H=H, noext_W=W, cam_param=int_mtx)
    xy2depth = {}
    int_mtx_pix = int_mtx * np.array([[W], [H], [1.]])
    LDI.graph['cam_param_pix'], LDI.graph['cam_param_pix_inv'] = int_mtx_pix, np.linalg.inv(
        int_mtx_pix)
    disp = 1. / (-depth)
    LDI.graph['hoffset'], LDI.graph['woffset'] = config['extrapolation_thickness'], config['extrapolation_thickness']
    LDI.graph['bord_up'], LDI.graph['bord_down'] = LDI.graph['hoffset'] + \
        0, LDI.graph['hoffset'] + H
    LDI.graph['bord_left'], LDI.graph['bord_right'] = LDI.graph['woffset'] + \
        0, LDI.graph['woffset'] + W
    for idx in range(H):
        for idy in range(W):
    ...
    for x, y, d in LDI.nodes:
        two_nes = [ne for ne in [(x+1, y), (x, y+1)] if ne[0] <
                   LDI.graph['bord_down'] and ne[1] < LDI.graph['bord_right']]
        [LDI.add_edge((ne[0], ne[1], xy2depth[ne][0]), (x, y, d))
         for ne in two_nes]
...
def tear_edges(mesh, threshold=0.00025, xy2depth=None):
    ...
    for edge in mesh.edges:
        if abs(mesh_nodes[edge[0]]['disp'] - mesh_nodes[edge[1]]['disp']) > threshold:
            ...
            near, far = edge if abs(edge[0][2]) < abs(
                edge[1][2]) else edge[::-1]

            mesh_nodes[far]['near'] = [] if mesh_nodes[far].get(
                'near') is None else mesh_nodes[far]['near'].append(near)
            mesh_nodes[near]['far'] = [] if mesh_nodes[near].get(
                'far') is None else mesh_nodes[near]['far'].append(far)
    ...
    dang_horizon = np.where(np.roll(remove_horizon, 1, 0) +
                            np.roll(remove_horizon, -1, 0) - remove_horizon == 2)
    dang_vertical = np.where(np.roll(
        remove_vertical, 1, 1) + np.roll(remove_vertical, -1, 1) - remove_vertical == 2)

    def horizon_condition(
        x, y): return mesh.graph['bord_up'] + 1 <= x < mesh.graph['bord_down'] - 1
    def vertical_condition(
        x, y): return mesh.graph['bord_left'] + 1 <= y < mesh.graph['bord_right'] - 1
...
def reproject_3d_int_detail_FB(sx, sy, z, w_offset, h_offset, mesh):
    ...
    ray = np.array([(-1. + 2. * ((sy+0.5-w_offset)/(mesh.graph['W'] - 1))) * mesh.graph['tan_hFov'],
                    (1. - 2. * (sx+0.5-h_offset) /
                     (mesh.graph['H'] - 1)) * mesh.graph['tan_vFov'],
                    -1])
...
def reproject_3d_int(sx, sy, z, mesh):
    ...
    if k[0, 2] > 0:
        k = np.linalg.inv(k)
    ray = np.dot(k, np.array(
        [sy-mesh.graph['woffset'], sx-mesh.graph['hoffset'], 1]).reshape(3, 1))
...
def generate_init_node(mesh, config, min_node_in_cc):
    ...
    for node in remove_nodes:
        far_nodes = [] if mesh_nodes[node].get(
            'far') is None else mesh_nodes[node]['far']
        ...
        near_nodes = [] if mesh_nodes[node].get(
            'near') is None else mesh_nodes[node]['near']
...
def reassign_floating_island(mesh, info_on_pix, image, depth):
    ...
    def is_inside(x, y, xmin, xmax, ymin,
                  ymax): return xmin <= x < xmax and ymin <= y < ymax
    def get_cross_nes(x, y): return [
    ...
    for i in range(1, label_lost_map.max()+1):
        ...
        edge_id, edge_nodes = sorted(
            [*surr_edge_ids.items()], key=lambda x: len(x[1]), reverse=True)[0]
        ...
        for ne in four_nes:
            for info in info_on_pix[ne]:
                assert mesh.has_node(
                    (ne[0], ne[1], info['depth'])), "No node_key"
...
def group_edges(LDI, config, image, remove_conflict_ordinal, spdb=False):
    ...
    '''
    def add_new_node(G, node): return None if G.has_node(
        node) else G.add_node(node)
    def add_new_edge(G, node_a, node_b): return None if G.has_edge(
        node_a, node_b) else G.add_edge(node_a, node_b)
        ...
        for edge_id, discont_cc in enumerate(discont_ccs):
            ...
            for discont_node in discont_cc:
                near_flag = True if key_exist(
                    LDI.nodes[discont_node], 'far') else near_flag
                far_flag = True if key_exist(
                    LDI.nodes[discont_node], 'near') else far_flag
            ...
            if far_flag and near_flag:
                    ...
                    if abs(ordinal_relation) < len([*discont_graph.neighbors(discont_node)]):
                        ...
                        for ne_node in discont_graph.neighbors(discont_node):
                            remove_flag = (near_side and not(key_exist(LDI.nodes[ne_node], 'far'))) or \
                                          (not near_side and not(
                                              key_exist(LDI.nodes[ne_node], 'near')))
                            remove_edges += [(discont_node, ne_node)
                                             ] if remove_flag else []
                    else:
                ...
                def is_redun_near(xx): return len(
                    xx) == 1 and xx[0] in remove_nodes and key_exist(LDI.nodes[xx[0]], 'far')
    ...
    for valid_edge_id, valid_edge_cc in enumerate(edge_ccs):
        for valid_edge_node in valid_edge_cc:
            ...
            if len([*edge_mesh.neighbors(valid_edge_node)]) == 1:
                num_ne = 1
                if num_ne == 1:
                    end_maps[valid_edge_node[0],
                             valid_edge_node[1]] = valid_edge_node[2]
    ...
    for nx, ny in zip(nxs, nys):
        ...
        for fne in four_nes:
            if mesh_nodes[(fne[0], fne[1], end_maps[fne[0], fne[1]])].get('edge_id') is None:
                ...
                ne_id = mesh_nodes[(
                    fne[0], fne[1], end_maps[fne[0], fne[1]])]['edge_id']
    ...
    for nx, ny in zip(nxs, nys):
        ...
        for fne in four_nes:
            if mesh.has_node((fne[0], fne[1], end_maps[fne[0], fne[1]])):
                node_a, node_b = (
                    fne[0], fne[1], end_maps[fne[0], fne[1]]), (nx, ny, end_maps[nx, ny])
                mesh.add_edge(node_a, node_b)
                mesh_nodes[node_b]['must_connect'] = set() if mesh_nodes[node_b].get(
                    'must_connect') is None else mesh_nodes[node_b]['must_connect']
                ...
                mesh_nodes[node_a]['must_connect'] = set() if mesh_nodes[node_a].get(
                    'must_connect') is None else mesh_nodes[node_a]['must_connect']
    ...
    for valid_edge_id, valid_edge_cc in enumerate(edge_ccs):
        for valid_edge_node in valid_edge_cc:
            ...
            if edge_mesh.has_node(valid_edge_node) is True:
                if len([*edge_mesh.neighbors(valid_edge_node)]) == 1:
                    end_maps[valid_edge_node[0],
                             valid_edge_node[1]] = valid_edge_id
    nxs, nys = np.where(end_maps > -1)
    point_to_adjoint = {}
    for nx, ny in zip(nxs, nys):
        adjoint_edges = set([end_maps[x, y] for x, y in [(
            nx + 1, ny), (nx - 1, ny), (nx, ny + 1), (nx, ny - 1)] if end_maps[x, y] != -1])
...
def judge_dangle(mark, mesh, node):
    ...
    mesh_neighbors = [xx for xx in mesh_neighbors if 0 < xx[0]
                      < mesh.graph['H'] - 1 and 0 < xx[1] < mesh.graph['W'] - 1]
...
def remove_dangling(mesh, edge_ccs, edge_mesh, info_on_pix, image, depth, config):
    ...
    for edge_cc_id, valid_edge_cc in enumerate(tmp_edge_ccs):
        ...
        largest_cc = sorted(four_ccs, key=lambda x: (
            len(x), -np.sum([abs(xx[2] - hz) for xx in x])))[-1]
        if len(largest_cc) < 2:
            for ne in four_nes:
                mesh.add_edge(single_edge_node, ne)
        else:
            mesh.remove_edges_from([(single_edge_node, ne)
                                    for ne in mesh.neighbors(single_edge_node)])
            ...
            mesh = refresh_node(
                single_edge_node, mesh.node[single_edge_node], new_node, dict(), mesh)
    ...
    for node in conn_1_nodes:
        ...
        eight_nes = set([(x, y, info_on_pix[(x, y)][0]['depth']) for x, y in [(hx + 1, hy), (hx - 1, hy), (hx, hy + 1), (hx, hy - 1),
                                                                              (hx + 1, hy + 1), (hx - 1, hy - 1), (hx - 1, hy + 1), (hx + 1, hy - 1)]
                         if info_on_pix.get((x, y)) is not None])
        self_nes = set([ne2 for ne1 in mesh.neighbors(node)
                        for ne2 in mesh.neighbors(ne1) if ne2 in eight_nes])
        eight_nes = [*(eight_nes - self_nes)]
        sub_mesh = mesh.subgraph(eight_nes).copy()
        ccs = netx.connected_components(sub_mesh)
        largest_cc = sorted(ccs, key=lambda x: (
            len(x), -np.sum([abs(xx[0] - node[0]) + abs(xx[1] - node[1]) for xx in x])))[-1]
    ...
    for cc in ccs:
        ...
        for node in candidate_nodes:
            ...
            try:
                ...
                import pdb
                pdb.set_trace()
            largest_cc = [xx for xx in ne_cc if abs(
                xx[0] - node[0]) + abs(xx[1] - node[1]) == 1]
            mesh.remove_edges_from([(xx, node) for xx in mesh.neighbors(node)])
            re_depth = {'value': 0, 'count': 0}
            for cc_node in largest_cc:
                re_depth['value'] += cc_node[2]
...
def context_and_holes(mesh, edge_ccs, config, specific_edge_id, specific_edge_loc, depth_feat_model,
    ...
    for edge_id, edge_cc in enumerate(edge_ccs):
        ...
        for edge_node in edge_cc:
            raw_intouched_nodes = set(mesh_nodes[edge_node].get(
                'near')) if mesh_nodes[edge_node].get('near') is not None else set()
    ...
    forbidden_map = np.pad(forbidden_map, ((forbidden_len, forbidden_len),
                                           (forbidden_len, forbidden_len)), mode='constant').astype(np.bool)
    ...
    for edge_id, edge_cc in enumerate(edge_ccs):
            ...
            if i == 0:
                ...
                for node in tmp_mask_nodes:
                    ...
                    connect_point_id = np.bincount(
                        connect_node).argmax() if len(connect_node) > 0 else -1
                            ...
                            if inpaint_iter == 0:
                                ...
                                if mesh_nodes[ne].get('edge_id') is not None and \
                                    ...
                                    for intouched_far in accum_intouched_fars:
                                        ...
                                        intouched_map[intouched_far[0],
                                                      intouched_far[1]] = True
                                if mesh_nodes[ne].get('near') is not None:
                                    ...
                                    for intouched_near in intouched_nears:
                                        ...
                                        intouched_map[intouched_near[0],
                                                      intouched_near[1]] = True
        ...
        if inpaint_iter == 0:
            depth_dict = get_depth_from_maps(
                context_map, mask_map, context_depth, mesh.graph['H'], mesh.graph['W'], log_depth=config['log_depth'])
            ...
            depth_dict = depth_inpainting(None, None, None, None, mesh, config, union_size,
                                          depth_feat_model, None, given_depth_dict=depth_dict, spdb=False)
            near_depth_map, raw_near_depth_map = np.zeros(
                (mesh.graph['H'], mesh.graph['W'])), np.zeros((mesh.graph['H'], mesh.graph['W']))
            filtered_comp_far_cc, filtered_accomp_near_cc = set(), set()
            for node in cur_accomp_near_cc:
                near_depth_map[node[0], node[1]
                               ] = depth_dict['output'][node[0], node[1]]
        ...
        accomp_extend_context_ccs[edge_id] |= set(
            cur_accomp_near_cc).intersection(cur_mask_cc)
        extend_edge_ccs[edge_id] |= set(
            cur_accomp_near_cc).intersection(cur_mask_cc)
        ...
        extend_context_ccs[edge_id] = extend_context_ccs[edge_id] - \
            mask_ccs[edge_id] - accomp_extend_context_ccs[edge_id]
    if inpaint_iter == 0:
        all_ecnt_cc = set()
        for ecnt_id, ecnt_cc in enumerate(extend_context_ccs):
            ...
            for constraint_context_id in constraint_context_ids:
                constraint_context_cc = constraint_context_cc | context_ccs[
                    constraint_context_id] | erode_context_ccs[constraint_context_id]
                constraint_erode_context_cc = constraint_erode_context_cc | erode_context_ccs[
            ...
            for i in range(background_thickness):
                if i == 0:
                    ...
                    tmp_context_map = np.zeros(
                        (mesh.graph['H'], mesh.graph['W'])).astype(np.bool)
                    ...
                    if vis_edge_id is not None and ecnt_id == vis_edge_id:
                        ...
                        ax2.imshow(init_invalid_context_map *
                                   1 + tmp_context_map * 2)
                        plt.show()
                        import pdb
                        pdb.set_trace()
            ...
            _, tmp_label_map = cv2.connectedComponents(
                (init_invalid_context_map | tmp_context_map).astype(np.uint8), connectivity=8)
            tmp_label_ids = set(
            ...
            extend_context_ccs[ecnt_id] = extend_context_ccs[ecnt_id] - \
                extend_erode_context_ccs[ecnt_id] - erode_context_ccs[ecnt_id]
            tmp_context_cc = context_ccs[ecnt_id] - \
                extend_erode_context_ccs[ecnt_id] - erode_context_ccs[ecnt_id]
            if len(tmp_context_cc) > 0:
                context_ccs[ecnt_id] = tmp_context_cc
            tmp_mask_cc = tmp_mask_cc - \
                context_ccs[ecnt_id] - erode_context_ccs[ecnt_id]
...
def DL_inpaint_edge(mesh,
    ...
    def edge_condition(x, m): return m.nodes[x].get(
        'far') is not None and len(m.nodes[x].get('far')) > 0
    ...
    for edge_id, (context_cc, mask_cc, erode_context_cc, extend_context_cc, edge_cc) in enumerate(zip(context_ccs, mask_ccs, erode_context_ccs, extend_context_ccs, edge_ccs)):
        ...
        mask_size = dilate_valid_size(
            mask_size, edge_dict['mask'], dilate=[20, 20])
        ...
        if require_depth_edge(patch_edge_dict['edge'], patch_edge_dict['mask']) and inpaint_iter == 0:
            ...
            tensor_edge_dict['output'] = (depth_edge_output > config['ext_edge_threshold']).float(
            ...
            if ((depth_edge_output > config['ext_edge_threshold']).float() * tensor_edge_dict['mask']).max() > 0:
                try:
                    edge_dict['fpath_map'], edge_dict['npath_map'], break_flag, npaths, fpaths, invalid_edge_id = \
                        clean_far_edge_new(edge_dict['output'], end_depth_maps, edge_dict['mask'],
                                           edge_dict['context'], mesh, info_on_pix, edge_dict['self_edge'], inpaint_iter, config)
                except:
                    import pdb
                    pdb.set_trace()
                ...
                if config.get('repeat_inpaint_edge') is True:
                    for _ in range(2):
                        tmp_input_edge = (
                            (edge_dict['npath_map'] > -1) + edge_dict['edge']).clip(0, 1)
                        patch_tmp_input_edge = crop_maps_by_size(
                            union_size, tmp_input_edge)[0]
                        tensor_input_edge = torch.FloatTensor(
                            patch_tmp_input_edge)[None, None, ...]
                        depth_edge_output = depth_edge_model.forward_3P(tensor_edge_dict['mask'],
                                                                        tensor_edge_dict['context'],
                                                                        tensor_edge_dict['rgb'],
                                                                        tensor_edge_dict['disp'],
                        ...
                        full_depth_edge_output[union_size['x_min']:union_size['x_max'], union_size['y_min']:union_size['y_max']] = \
                            depth_edge_output
                        edge_dict['fpath_map'], edge_dict['npath_map'], break_flag, npaths, fpaths, invalid_edge_id = \
                            clean_far_edge_new(full_depth_edge_output, end_depth_maps, edge_dict['mask'], edge_dict[
                                               'context'], mesh, info_on_pix, edge_dict['self_edge'], inpaint_iter, config)
                for nid in npaths.keys():
                    ...
                    if start_mx == -1:
                        import pdb
                        pdb.set_trace()
                    valid_end_pt = () if end_mx == -1 else (end_mx, end_my,
                    ...
                    if len(valid_end_pt) > 0:
                        new_edge_info = dict(fpath=fpath[::-1],
                                             npath=npath[::-1],
                                             cont_end_pts=(start_mx, start_my, info_on_pix[(
                                                 start_mx, start_my)][0]['depth']),
                                             mask_id=edge_id,
                                             comp_edge_id=nid,
    ...
    for edge_id, (context_cc, mask_cc, erode_context_cc, extend_context_cc, edge_cc) in enumerate(zip(context_ccs, mask_ccs, erode_context_ccs, extend_context_ccs, edge_ccs)):
        ...
        if require_depth_edge(patch_edge_dict['edge'], patch_edge_dict['mask']) and inpaint_iter == 0:
            edge_dict['fpath_map'], edge_dict['npath_map'] = edge_dict['fpath_map'] * \
                0 - 1, edge_dict['npath_map'] * 0 - 1
            ...
            for end_pt in end_pts:
                ...
                other_infos = [xx for xx in cur_edge_infos if xx['mask_id'] != edge_id and len(
                    xx['cont_end_pts']) > 0]
                if len(cur_info['cont_end_pts']) > 0 or (len(cur_info['cont_end_pts']) == 0 and len(other_infos) == 0):
                    for fnode in cur_info['fpath']:
                        edge_dict['fpath_map'][fnode[0],
                                               fnode[1]] = cur_info['comp_edge_id']
                    for fnode in cur_info['npath']:
                        edge_dict['npath_map'][fnode[0],
                                               fnode[1]] = cur_info['comp_edge_id']
            fnmap = edge_dict['fpath_map'] * 1
            fnmap[edge_dict['npath_map'] != -
                  1] = edge_dict['npath_map'][edge_dict['npath_map'] != -1]
            for end_pt in end_pts:
                cur_edge_infos = edges_infos[(end_pt[0], end_pt[1])]
                cur_info = [
                    xx for xx in cur_edge_infos if xx['mask_id'] == edge_id][0]
                ...
                if len(cur_info['cont_end_pts']) == 0 and len(other_infos) > 0:
                    other_infos = sorted(other_infos, key=lambda aa: abs(
                        abs(aa['cont_end_pts'][2]) - abs(cur_depth)))
                    for other_info in other_infos:
                        tmp_fmap, tmp_nmap = np.zeros(
                            (mesh.graph['H'], mesh.graph['W'])) - 1, np.zeros((mesh.graph['H'], mesh.graph['W'])) - 1
                    ...
                    if min(tmp_fmap.max(), tmp_nmap.max()) != -1:
                        ...
                        discard_map = ((tmp_nmap != -1).astype(np.uint8) +
                                       (tmp_fmap != -1).astype(np.uint8)) * edge_dict['mask']
                    else:
                        for fnode in cur_info['fpath']:
                            edge_dict['fpath_map'][fnode[0],
                                                   fnode[1]] = cur_info['comp_edge_id']
            ...
            if edge_dict['npath_map'].min() == 0 or edge_dict['fpath_map'].min() == 0:
                import pdb
                pdb.set_trace()
            edge_dict['output'] = (edge_dict['npath_map'] > -1) * \
                edge_dict['mask'] + edge_dict['context'] * edge_dict['edge']
        ...
        depth_dict = depth_inpainting(context_cc, extend_context_cc, erode_context_cc |
                                      extend_erode_context_ccs[edge_id], mask_cc, mesh, config, union_size, depth_feat_model, edge_dict['output'])
        ...
        for near_id in np.unique(edge_dict['npath_map'])[1:]:
            refine_depth_output = refine_depth_around_edge(refine_depth_output.copy(),
                                                           (edge_dict['fpath_map'] == near_id).astype(
                                                               np.uint8) * edge_dict['mask'],
                                                           (edge_dict['fpath_map'] == near_id).astype(
                                                               np.uint8),
                                                           (edge_dict['npath_map'] == near_id).astype(
                                                               np.uint8) * edge_dict['mask'],
                ...
                depth_dict['context'],
                config)
        depth_dict['output'][depth_dict['mask'] >
                             0] = refine_depth_output[depth_dict['mask'] > 0]
        ...
        if frac < 1:
            ...
            resize_rgb_dict['rgb'] = torch.nn.functional.interpolate(resize_rgb_dict['rgb'],
                                                                     scale_factor=frac,
                                                                     mode='area')
            resize_rgb_dict['rgb'] = resize_rgb_dict['rgb'] * \
                resize_rgb_dict['context']
        ...
        with torch.no_grad():
            ...
            if config.get('gray_image') is True:
                rgb_output = rgb_output.mean(
                    1, keepdim=True).repeat((1, 3, 1, 1))
            rgb_output = rgb_output.cpu()
        resize_rgb_dict['output'] = rgb_output * \
            resize_rgb_dict['mask'] + resize_rgb_dict['rgb']
        ...
        patch_rgb_dict['output'] = tensor_rgb_dict['output'].data.cpu(
            ...
            if rgb_dict['mask'][node[0], node[1]] > 0:
                for info in info_on_pix[(node[0], node[1])]:
                    if abs(info['depth']) == abs(node[2]):
                        info['update_color'] = (
                            rgb_dict['output'][node[0], node[1]] * 255).astype(np.uint8)
        if frac < 1.:
        ...
        for hx, hy in zip(hxs, hys):
                ...
                for info in info_on_pix.get((node[0], node[1])):
                    if info.get('inpaint_id') is None or abs(info['inpaint_id'] < mesh.nodes[(hx, hy)]['inpaint_id']):
                        pre_depth = info['depth'] if info.get(
                            'real_depth') is None else info['real_depth']
            ...
            if not(mesh.has_node(node)):
                ...
                mesh, _, _ = refresh_node((node[0], node[1]), mesh.nodes[(
                    node[0], node[1])], node, ndict, mesh, stime=True)
        ...
        for erode_node in erode_context_cc:
            for info in info_on_pix[(erode_node[0], erode_node[1])]:
                if info['depth'] == erode_node[2]:
                    ...
                    np_image[(erode_node[0], erode_node[1])
                             ] = info['update_color']
...
def write_ply(checkPointList,
    ...
    input_mesh, info_on_pix = generate_init_node(
        input_mesh, config, min_node_in_cc=200)
    ...
    if config['extrapolate_border'] is True:
        ...
        edge_ccs, input_mesh, edge_mesh = \
            group_edges(input_mesh, config, image,
        ...
        edge_ccs, input_mesh, edge_mesh = \
            group_edges(input_mesh, config, image,
                        remove_conflict_ordinal=True, spdb=False)
        input_mesh = remove_redundant_edge(
            input_mesh, edge_mesh, edge_ccs, info_on_pix, config, redundant_number=config['redundant_number'], spdb=False)
        input_mesh, depth, info_on_pix = update_status(
            input_mesh, info_on_pix, depth)
        edge_ccs, input_mesh, edge_mesh = group_edges(
            input_mesh, config, image, remove_conflict_ordinal=True)
        input_mesh = combine_end_node(input_mesh, edge_mesh, edge_ccs, depth)
        input_mesh = remove_redundant_edge(input_mesh, edge_mesh, edge_ccs, info_on_pix,
                                           config, redundant_number=config['redundant_number'], invalid=True, spdb=False)
        input_mesh, depth, info_on_pix = update_status(
            input_mesh, info_on_pix, depth)
        edge_ccs, input_mesh, edge_mesh = group_edges(
            input_mesh, config, image, remove_conflict_ordinal=True)
        ...
        def edge_condition(x, m): return m.nodes[x].get(
            'far') is not None and len(m.nodes[x].get('far')) > 0
        edge_map = get_map_from_ccs(
            edge_ccs, input_mesh.graph['H'], input_mesh.graph['W'], input_mesh, edge_condition)
    ...
    context_ccs, mask_ccs, broken_mask_ccs, edge_ccs, erode_context_ccs, init_mask_connect, \
        ...
        context_and_holes(input_mesh, new_edge_ccs, config, specific_edge_id,
                          specific_edge_loc, depth_feat_model, connect_points_ccs, inpaint_iter=1)
    ...
    k_00, k_02, k_11, k_12 = \
        ...
        input_mesh.graph['cam_param_pix_inv'][1,
                                              1], input_mesh.graph['cam_param_pix_inv'][1, 2]
    ...
    for pix_xy, pix_list in info_on_pix.items():
        ...
        for pix_idx, pix_info in enumerate(pix_list):
            pix_depth = pix_info['depth'] if pix_info.get(
                'real_depth') is None else pix_info['real_depth']
            # ここでif x,y=checklist ならstr_ptというのを保存しておく
            str_pt = [out_fmt(x, ply_flag) for x in reproject_3d_int_detail(pix_xy[0], pix_xy[1], pix_depth,
                                                                            k_00, k_02, k_11, k_12, w_offset, h_offset)]  # pix_xy[0]がsxでx[1]がsyでy

            # add
            for index in range(len(checkPointList)):
                if checkPointList[index] == pix_xy:
                    # print("fit!!!!!!")
                    if not str(index) in FPDict:
                        FPDict[str(index)] = [str_pt[0], str_pt[1], str_pt[2]]
                        # print("add!!!!!!!!!!!")
                    else:
                        if FPDict[str(index)][2] > str_pt[2]:
                            FPDict.pop(str(index))
                            FPDict[str(index)] = [str_pt[0],
                                                  str_pt[1], str_pt[2]]
                        else:
                            pass
                    # str(index)+" "+str(str_pt[0])+" "+str(str_pt[1])+" "+str(str_pt[2]))
                    #color_flag = 1
            #print("FPDict", FPDict)
            ...
            if pix_info.get('overlap_number') is not None:
                ...
                str_color = [out_fmt(x, ply_flag)
                             for x in pix_info['color'].tolist()]
            ...
            vertex_id += 1
            if ply_flag is True:
                node_str_list.append(' '.join(str_pt) +
                                     ' ' + ' '.join(str_color) + '\n')
            else:
    ...
    if config['save_ply'] is True:
        ...
        with open(ply_name, 'w') as ply_fi:
            ...
            ply_fi.write('comment hFov ' +
                         str(float(input_mesh.graph['hFov'])) + '\n')
            ply_fi.write('comment vFov ' +
                         str(float(input_mesh.graph['vFov'])) + '\n')
            ...
            for k, v in FPDict.items():
                #print(k, v)
                #FPStr = k + " "+str(v[0]) + " " + str(v[1]) + " " + str(v[2])
                FPnp = np.array([k, v[0], v[1], v[2]])
                FPList.append(FPnp)

            #FPSave = '・'.join(FPList)
            FPSave = np.array(FPList)
            #np.save("./FP3D/")
            #ply_fi.write("comment featurePoint・ "+FPSave+'\n')

            ply_fi.write('end_header\n')
            ply_fi.writelines(node_str_list)
            ply_fi.writelines(str_faces)
        ply_fi.close()
        return input_mesh,FPSave
    else:
...
class Canvas_view():
    def __init__(self,
        ...
        self.canvas = scene.SceneCanvas(bgcolor=bgcolor, size=(
            canvas_size*factor, canvas_size*factor))
...
def output_3d_photo(verts, colors, faces, Height, Width, hFov, vFov, tgt_poses, video_traj_types, ref_pose,
                    ...
                    # output_dir, int_mtx, config, videos_poses, video_basename, original_H=None, original_W=None,
    ...
    backup_img, backup_all_img, all_img_wo_bound = img.copy(), img.copy() * \
        0, img.copy() * 0
    img = cv2.resize(img, (int(img.shape[1] / init_factor), int(
        img.shape[0] / init_factor)), interpolation=cv2.INTER_AREA)
    if border is None:
        border = [0, img.shape[0], 0, img.shape[1]]
    H, W = cam_mesh.graph['H'], cam_mesh.graph['W']
    if (cam_mesh.graph['original_H'] is not None) and (cam_mesh.graph['original_W'] is not None):
        aspect_ratio = cam_mesh.graph['original_H'] / \
            cam_mesh.graph['original_W']
    else:
        aspect_ratio = cam_mesh.graph['H'] / cam_mesh.graph['W']
    if aspect_ratio > 1:
        img_h_len = cam_mesh.graph['H'] if cam_mesh.graph.get(
            'original_H') is None else cam_mesh.graph['original_H']
        ...
        img_w_len = cam_mesh.graph['W'] if cam_mesh.graph.get(
            'original_W') is None else cam_mesh.graph['original_W']
        img_h_len = img_w_len * aspect_ratio
        anchor = [int(max(0, int((img.shape[0])//2 - img_h_len//2))),
                  int(min(int((img.shape[0])//2 +
                              img_h_len//2), (img.shape[0])-1)),
                  0,
                  img.shape[1]]
    ...
    for video_pose, video_traj_type in zip(videos_poses, video_traj_types):
        ...
        for tp_id, tp in enumerate(video_pose):
            ...
            if 'dolly' in video_traj_type:
                new_fov = float((np.arctan2(plane_width, np.array(
                    [np.abs(new_mean_loc_depth)])) * 180. / np.pi) * 2)
            ...
            if any(np.array(config['crop_border']) > 0.0):
                ...
                img = cv2.resize(
                    img, (W_c, H_c), interpolation=cv2.INTER_CUBIC)
        ...
        clip.write_videofile(os.path.join(
            output_dir, video_basename + '_' + video_traj_type + '.mp4'), fps=config['fps'])
