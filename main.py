...
from readFP import readPoint, getName, readCVMatching
#args = sys.argv
#img1Path = args[1]
# img2=args[2]
#img1Path = input()
...
# sample listはなんかMidasをした結果出てきたdepth画像とか、imageの画像とか
for idx in tqdm(range(len(sample_list))):
    ...
    if config['require_midas'] is True:
        run_depth([sample['ref_img_fi']], config['src_folder'], config['depth_folder'],
                  config['MiDaS_model_ckpt'], MonoDepthNet, MiDaS_utils, target_w=600)
    # midasを走らせる
    ...
    mean_loc_depth = depth[depth.shape[0] //
                           2, depth.shape[1] // 2]  # //切り捨て除算
    # デプスマップ中央の深度
    ...
    if not(config['load_ply'] is True and os.path.exists(mesh_fi)):
        ...
        depth_edge_model = depth_edge_model.to(device)
        #print("depth_edge model{}".format(device))
        ...
        #checkPointList = [(320, 358), (128, 458)]
        # glob.glob("./image")
        #name = os.path.basename(img1Path)
        name = os.path.splitext(os.path.basename(img1Path))[0]
        # print(name)

        #FPList = readPoint("./FP/FP_"+name+".txt")
        FPList = readPoint("./FP/FP_" + name + ".txt")
        FPList = readCVMatching("./FP/FP_" + name + ".npy")

        rt_info, FPnp = write_ply(FPList,
                                  image,
                                  depth,
                                  sample['int_mtx'],
                                  mesh_fi,
    ...
    if config['save_ply'] is True or config['load_ply'] is True:
        verts, colors, faces, Height, Width, hFov, vFov = read_ply(
            mesh_fi)
        #print("if: feature", featurePointStr)
